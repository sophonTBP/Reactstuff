...
////map over input and order it into list with array.map()
...
/////generate unique keys for each entries in an array(the username could also be used as a key)
...
function Frameworks() {
  const renderFrameworks = frontEndFrameworks.map((framework,index) => <li key={"200"+(index+1)}>{framework}</li>)
  return (
    <div>
...
///create keys and filter an array (the username could also be used as a key)
...
class MyComponent extends React.Component {
  ...
  render() {
    ...
    const renderOnline = usersOnline.map((user,index) => <li key={"200"+(index+1)}>{user.username}</li>); // Change this line
    return (
      <div>
...
////////server side rendering (don't Effin' forget the tags"</>" when rendering an element you Willy)
